[CmdletBinding()]
param(
    [ValidateSet("spring-boot", "spring-webflux", "rust-java-rest")]
    [string] $ApplicationKind = "spring-boot",
    [ValidateSet("tomcat", "jetty", "undertow", "reactor-netty")]
    [string] $ServletContainer = "tomcat",
    [string] $RequiredRestVersion = "4.6.2",
...
if ($IsRustJavaRest -and $ServletContainer -ne "tomcat") {
    throw "ServletContainer does not apply to the rust-java-rest application kind."
}
if ($IsWebFlux -and $ServletContainer -ne "reactor-netty") {
    throw "spring-webflux requires ServletContainer=reactor-netty."
}
if (-not $IsWebFlux -and -not $IsRustJavaRest -and $ServletContainer -eq "reactor-netty") {
    throw "spring-boot Servlet MVC requires tomcat, jetty, or undertow."
}
$springRootName = if ($IsWebFlux) { "webflux-app" } else { "spring-app" }
$SpringRoot = Join-Path $ScriptDir $springRootName
$FrameworkRoot = [IO.Path]::GetFullPath((Join-Path $ProjectRoot "..\rust-java-rest"))
$ContextName = if ($IsRustJavaRest) {
    "context"
} elseif ($IsWebFlux) {
    "webflux-context"
} else {
...
$ResultPattern = if ($IsRustJavaRest) {
    "results\rest_gate_{0}"
} elseif ($IsWebFlux) {
    "results\webflux_gate_{0}"
} else {
    "results\spring_gate_{0}"
}
...
$AppImage = if ($IsRustJavaRest) {
    ...
    "java-rust-glowroot-agent:webflux-benchmark"
} else {
    "java-rust-glowroot-agent:spring-benchmark-$ServletContainer"
}
$CollectorImage = "java-rust-glowroot-agent:mock-collector"
$RunnerImage = "curlimages/curl:8.12.1"
$WrkImage = "williamyeh/wrk:latest"
$namePrefix = if ($IsRustJavaRest) { "rest" } elseif ($IsWebFlux) { "webflux" } else { "spring" }
$Baseline = "$namePrefix-glowroot-baseline"
$Candidate = "$namePrefix-glowroot-candidate"
$Collector = "$namePrefix-glowroot-collector"
$LoadRunner = "$namePrefix-glowroot-load-runner"
$ContainerCpuSets = @{}
...
function Prepare-Build {
    ...
    if ($IsRustJavaRest) {
        ...
        Copy-Item (Join-Path $ScriptDir "rest-app\src") `
                (Join-Path $Context "src") -Recurse
        Invoke-Checked docker @("build", "-f", "app.Dockerfile", "-t", $AppImage, ".") `
                $ScriptDir | Out-Null
    } else {
        $springArtifact = if ($IsWebFlux) {
            "spring-webflux-glowroot-benchmark-1.0.0.jar"
        } else {
            "spring-glowroot-benchmark-1.0.0.jar"
        }
        Copy-Item (Join-Path $SpringRoot "target\$springArtifact") `
...
function Test-PairedWarmupStability([hashtable] $SamplesByVariant) {
    if ($PairedWarmupRatioGate) {
        foreach ($endpoint in $endpoints) {
            $ratios = Get-PairedWarmupRatioSamples `
                    -BaselineSamples @($SamplesByVariant["baseline"][$endpoint]) `
                    -CandidateSamples @($SamplesByVariant["candidate"][$endpoint])
            $decision = Get-ReactorWarmupDecision `
                    -Samples $ratios `
                    -WindowRounds $MinWarmupRounds `
                    -MaximumRobustTrendPercent $MaxWarmupRobustTrendPercent `
...
function Get-PairedWarmupRatioSamples(
        [double[]] $BaselineSamples,
        [double[]] $CandidateSamples) {
    if ($BaselineSamples.Count -ne $CandidateSamples.Count) {
        throw "Paired warmup sample counts differ: baseline=$($BaselineSamples.Count) candidate=$($CandidateSamples.Count)."
    }
    $ratios = [Collections.Generic.List[double]]::new()
    for ($index = 0; $index -lt $BaselineSamples.Count; $index++) {
        if ($BaselineSamples[$index] -le 0) {
            throw "Paired warmup baseline RPS must be positive at index $index."
        }
        $ratios.Add(100.0 * $CandidateSamples[$index] / $BaselineSamples[$index])
    }
    return @($ratios)
}

function Assert-StabilizedPairedWarmup(
        [int] $Pair,
        [string] $Endpoint,
    ...
    if ($BaselineSamples.Count -ne $expectedRounds -or $CandidateSamples.Count -ne $expectedRounds) {
        throw "Endpoint $Endpoint paired warmup has baseline=$($BaselineSamples.Count) candidate=$($CandidateSamples.Count) samples; expected $expectedRounds."
    }
    $ratios = Get-PairedWarmupRatioSamples `
    ...
    for ($round = 2 * $MinWarmupRounds; $round -le $expectedRounds; $round++) {
        $decision = Get-ReactorWarmupDecision `
                -Samples @($ratios | Select-Object -First $round) `
                -WindowRounds $MinWarmupRounds `
                -MaximumRobustTrendPercent $MaxWarmupRobustTrendPercent `
                -MaximumBorderlineRobustTrendPercent $MaxWarmupBorderlineRobustTrendPercent `
                -MaximumBorderlineMedianShiftPercent $MaxWarmupBorderlineMedianShiftPercent `
                -MaximumMedianAbsoluteDeviationPercent $MaxWarmupMedianAbsoluteDeviationPercent
        if ($null -eq $firstStableRound -and $decision.passed) { $firstStableRound = $round }
    }
    ...
    $script:warmups.Add([pscustomobject]@{
        pair = $Pair
        variant = "paired-ratio"
        endpoint = $Endpoint
        measurement = "candidate_to_baseline_rps_ratio_pct"
        ...
        rps_samples = @($ratios)
        baseline_rps_samples = @($BaselineSamples)
        candidate_rps_samples = @($CandidateSamples)
        gate = if ($warmupPassed) { "PASS" } else { "FAIL" }
    })
    if (-not $warmupPassed -and -not $DevelopmentQuickMode) {
        $ratioText = @($ratios | ForEach-Object {
            $_.ToString("F3", [Globalization.CultureInfo]::InvariantCulture)
        }) -join ", "
        throw ("Paired warmup ratio did not stabilize for pair=$Pair endpoint=$Endpoint " +
                ...
                "Candidate/baseline ratios: $ratioText.")
    }
}
...
function Start-App([string] $Name, [string] $CpuSet, [bool] $Enabled, [int] $Pair) {
    ...
    $agentIdPrefix = if ($IsRustJavaRest) {
        "rust-java-rest-benchmark"
    } elseif ($IsWebFlux) {
        "spring-webflux-benchmark"
    } else {
        "spring-benchmark"
    }
    $applicationName = if ($IsRustJavaRest) {
        "rust-java-rest-benchmark"
    } elseif ($IsWebFlux) {
        "spring-webflux-glowroot-benchmark"
    } else {
        "spring-glowroot-benchmark"
    }
    ...
    $singleCpuSetOwnsTheLimit = $IsLinux `
            -and $PinSingleCpuQuotaToOneLogicalCpu `
            -and [Math]::Abs($CpuLimit - 1.0) -lt 0.0001 `
            -and @(ConvertFrom-ReactorCpuSet -CpuSet $executionCpuSet).Count -eq 1
    # A one-CPU cpuset is already a hard cap. Adding an equal CFS quota creates
    # periodic throttling and makes short OpenJ9 A/B samples scheduler-dependent.
    if (-not $singleCpuSetOwnsTheLimit) { $args += @("--cpus", "$CpuLimit") }
...
function Assert-SpringTelemetryAdapter([string] $Name, [bool] $Enabled) {
    ...
    $adapter = Invoke-Curl "http://${Name}:8080/internal/benchmark/telemetry-adapter" |
            ConvertFrom-Json
    if ($Enabled) {
        $expectedAdapter = if ($IsWebFlux) {
            "webflux-filter"
        } else {
            switch ($ServletContainer) {
                "tomcat" { "tomcat-valve" }
                "jetty" { "jetty-request-log" }
                "undertow" { "undertow-completion-listener" }
                default { throw "No optimized HTTP telemetry adapter contract exists for $ServletContainer." }
            }
        }
        if ($adapter.enabled -ne $true -or $adapter.httpAdapter -ne $expectedAdapter) {
            throw "Spring candidate did not select $expectedAdapter on $ServletContainer`: $($adapter | ConvertTo-Json -Compress)"
        }
        if ($adapter.fullLifecycle -ne $true) {
            throw "Spring candidate did not expose the complete HTTP lifecycle contract."
        }
        return
    }
    if ($adapter.enabled -ne $false -or $adapter.httpAdapter -ne "none") {
        throw "Spring baseline unexpectedly enabled telemetry: $($adapter | ConvertTo-Json -Compress)"
    }
}

function Assert-SpringLifecycleSmoke([string] $Name) {
    if ($IsRustJavaRest) { return }
    $expectations = @(
    ...
    foreach ($expectation in $expectations) {
        $status = ((& docker run --rm --network $Network $RunnerImage `
                -sS -o /dev/null -w "%{http_code}" `
                "http://${Name}:8080$($expectation.Path)" 2>&1) -join "").Trim()
        if ($LASTEXITCODE -ne 0 -or $status -ne $expectation.Status) {
            throw "$ApplicationKind/$ServletContainer lifecycle smoke failed for $($expectation.Path): expected $($expectation.Status), received $status."
        }
    }
}
...
if ($PairedWarmupRatioGate -and $SequentialVariants) {
    throw "PairedWarmupRatioGate requires parallel, core-swapped baseline/candidate execution."
}
...
try {
    for ($pair = 1; $pair -le $PairRepeats; $pair++) {
        ...
        if ($SequentialVariants) {
            ...
            foreach ($variant in $variants) {
                ...
                Assert-SpringLifecycleSmoke $name
        ...
        Assert-SpringLifecycleSmoke $Baseline
        Assert-SpringLifecycleSmoke $Candidate
        foreach ($variant in @("baseline", "candidate")) {
        ...
        if ($PairedWarmupRatioGate) {
            foreach ($endpoint in $endpoints) {
                Assert-StabilizedPairedWarmup `
                        -Pair $pair `
                        -Endpoint $endpoint `
                        -ConfirmationRoundsUsed $confirmationRoundsUsed `
                        -BaselineSamples @($warmupSamples["baseline"][$endpoint]) `
                        -CandidateSamples @($warmupSamples["candidate"][$endpoint])
            }
        } else {
            foreach ($variant in @("baseline", "candidate")) {
                foreach ($endpoint in $endpoints) {
    ...
    if (& docker ps -a --format "{{.Names}}" | Where-Object { $_ -eq $Collector }) {
        & docker cp "${Collector}:/tmp/glowroot-mock-report.json" `
                (Join-Path $Results "collector-report.json") 2>$null
    }
...
$collectorEvidencePath = Join-Path $Results "collector-report.json"
$collectorEvidence = $null
$collectorEvidencePassed = $true
if (-not $IsRustJavaRest) {
    if (-not (Test-Path -LiteralPath $collectorEvidencePath)) {
        $collectorEvidencePassed = $false
    } else {
        $collectorEvidence = Get-Content -Raw -LiteralPath $collectorEvidencePath | ConvertFrom-Json
        $collectorEvidencePassed = $collectorEvidence.healthy -eq $true `
                -and [long]$collectorEvidence.aggregate_transactions -gt 0 `
                -and @($collectorEvidence.transaction_types) -contains "Web" `
                -and @($collectorEvidence.transaction_names) -contains "/api/small"
    }
}
...
$passedAll = -not ($summary.gate -contains "FAIL") -and $startupDelta -le 10.0 `
        -and $steadyMemoryPassed -and $warmupGatePassed -and $collectorEvidencePassed
...
[ordered]@{
    ...
    application_kind = $ApplicationKind
    web_runtime = if ($IsRustJavaRest) { "rust-hyper" } else { $ServletContainer }
    ...
    warmup_stability = [ordered]@{
        ...
        gate_mode = if ($PairedWarmupRatioGate) {
            "paired_candidate_to_baseline_rps_ratio"
        } else {
            "absolute_rps_per_variant"
        }
        ...
        gate = if ($warmupGatePassed) { "PASS" } else { "FAIL" }
    }
    collector_evidence = if ($IsRustJavaRest) {
        [ordered]@{ required = $false; gate = "NOT_APPLICABLE" }
    } else {
        [ordered]@{
            required = $true
            healthy = if ($null -eq $collectorEvidence) { $false } else { [bool]$collectorEvidence.healthy }
            aggregate_transactions = if ($null -eq $collectorEvidence) {
                0
            } else {
                [long]$collectorEvidence.aggregate_transactions
            }
            transaction_names = if ($null -eq $collectorEvidence) {
                @()
            } else {
                @($collectorEvidence.transaction_names)
            }
            gate = if ($collectorEvidencePassed) { "PASS" } else { "FAIL" }
        }
    }
...
$reportTitle = if ($IsRustJavaRest) {
    "Rust-Java REST Agent Gate"
} elseif ($IsWebFlux) {
    "Spring WebFlux Agent Gate"
} else {
    "Spring Boot Agent Gate"
}
$compatibilityDescription = if ($IsRustJavaRest) {
    ...
    "Compatibility: Spring WebFlux on Reactor Netty using the optional bounded WebFilter adapter."
} else {
    "Compatibility: Spring Boot Servlet MVC on $ServletContainer using its direct completion adapter."
}
...
$lines.Add("Warmup gate: **$(if ($warmupGatePassed) { 'PASS' } else { 'FAIL' })**. Unstable warmup blocks quantitative performance claims even in development-only mode.")
    $lines.Add("Collector evidence: **$(if ($collectorEvidencePassed) { 'PASS' } else { 'FAIL' })**$(if ($IsRustJavaRest) { ' (covered by the dedicated embedded REST protocol gate).' } else { '; the candidate exported exact Web aggregates with the normalized /api/small route.' })")

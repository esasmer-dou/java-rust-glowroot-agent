...
The application and the two-thread `wrk` load runner each reserve a complete SMT sibling group. A
one-CPU application executes on one logical CPU inside that reserved group, preventing
sibling-parallel CFS quota oscillation while the other sibling remains isolated. Therefore

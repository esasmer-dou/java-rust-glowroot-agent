...
- Prepare the collector connection when the exporter starts and run the exporter below normal
  operating-system priority. First-minute DNS/TCP/HTTP/2/init work does not compete with an active
  request interval.
...
Rust-Java REST `4.5.1` already contains the embedded runtime. It needs no starter. Add the small
...
| Native platform | Windows x64 or Linux glibc x64 from the clean `rust-spring v4.5.1` revision |
...
load-generator saturation cannot be mistaken for application or agent instability. Docker polling
...
The Rust-Java matrix runs first and a failure skips Spring automatically. Throughput trend,
dispersion, p99, error, and memory limits remain unchanged.

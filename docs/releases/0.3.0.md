...
- Validate collector reachability when the exporter starts and run the exporter below normal
  operating-system priority. Embedded REST closes that startup probe and reconnects only for a
  bounded export window. The standalone Spring runtime reuses one bounded connection.
...
Rust-Java REST `4.5.2` already contains the embedded runtime. It needs no starter. Add the small
...
| Native platform | Windows x64 or Linux glibc x64 from the clean `rust-spring v4.5.2` revision |
